...
// Возвращает GET запрос в виде объекта
WB_API.parse_query();

```

# Краткое руководство по авторизации.
- Шаг 1. Запросить токен у нашей системы WB_API.users.get_auth_token
- Шаг 2. Позвать этот метод (WB_API.redirect_to_steam_auth) передав в него auth_token и если нужно редирект и хост
- Шаг 3. Пользователь оказался на странице авторизации в стиме.
- Шаг 4. Пользователь успешно авторизовался.
- Шаг 5. Стим перевел пользователя на страницу указанную в редиректе.
- Шаг 6. Вызвать WB_API.parse_query() и передать его в WB_API.users.auth как опции
- Шаг 7. Обработать ответ на авторизацию.
...
1) Нужно скачать эту библиотеку - [Dispatcher](https://github.com/chichenkov257/wsclient/blob/master/dispatcher.dev.js);
